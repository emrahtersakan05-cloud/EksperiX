...
export default function ImarDurumuSection({
  ...
  const [sekme, setSekme] = useState<"bilgiler" | "planNotlari">("bilgiler");
  const planNotlari = data.planNotlari ?? [];
  const doluAlan = (Object.keys(data) as (keyof ImarDurumuData)[]).filter(
    (k): k is ImarMetinAlani => k !== "planNotlari" && !KADASTRO_ALANLARI.includes(k) && Boolean(data[k].trim()),
  ).length;
  ...
  return (
    <div className="space-y-4">
      <div role="tablist" aria-label="İmar durumu" className="flex flex-wrap gap-1 rounded-xl bg-slate-100 p-1">
        {(
          [
            ["bilgiler", "İmar Durum Bilgileri"],
            ["planNotlari", "Plan Not Bilgileri"],
          ] as const
        ).map(([k, ad]) => {
          const aktif = sekme === k;
          return (
            ...
            >
              {ad}
              {k === "planNotlari" && planNotlari.length > 0 && (
                <span className="rounded-full bg-slate-200 px-1.5 py-0.5 text-[11px] tabular-nums text-slate-600">{planNotlari.length}</span>
              )}
            </button>
          );
        })}
      ...
      {sekme === "planNotlari" ? (
        <PlanNotlari notlar={planNotlari} onChange={(notlar) => onChange({ planNotlari: notlar })} />
      ) : (
        <>
