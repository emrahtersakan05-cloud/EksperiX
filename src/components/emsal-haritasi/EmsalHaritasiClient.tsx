...
export default function EmsalHaritasiClient({
  ...
  // Picking a row in the table below scrolls the map back into view.
  const mapPanelRef = useRef<HTMLDivElement>(null);
  function focusFromTable(id: string) {
    setFocusId(id);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-4 lg:h-[calc(100vh-8rem)] lg:min-h-[560px] lg:flex-row">
        ...
        <div ref={mapPanelRef} className="relative order-1 h-[60vh] min-h-[380px] overflow-hidden rounded-2xl border border-slate-100 lg:order-none lg:h-auto lg:min-h-0 lg:flex-1">
          <EmsalHaritaMap
      ...
      <EmsalAnalizSekmeleri
        records={listeKayitlari}
        cevre={cevre}
        focusId={focusId}
        onFocus={focusFromTable}
        canModify={canModify}
        onEdit={openEdit}
      />
    </div>
